...
'''Plotting of Correlation begins here'''

sel_countries_heat = ['France', 'India', 'China']

parameters_indi = ['2016', '2020']                   

# For loop for correlation heat map
for para2 in sel_countries_heat:
  ...
  df_x = pd.DataFrame({'Population, total':values_1,
                     'Population growth (annual %)':values_2,
                     'Urban population':values_3,
                     'Urban population growth (annual %)':values_4,
                     'Terrestrial and marine protected areas (% of total territorial area)':values_5,
                     'Population in urban agglomerations of more than 1 million (% of total population)':values_6,
                     })

  b = df_x.corr()
  fig, ax = plt.subplots(figsize = (10, 10))   
  sns.heatmap(b, cmap = "Reds", annot = True).set(title = "Correlation of selected parameters in " + para2)
  
  # You can change the cmap value to Blues, and so on to get different colored grid.